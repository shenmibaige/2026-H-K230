...
static void Q2CurveControl_StopForFinish(
    Q2CurveControl *control,
    CurveFinishTrigger trigger);

static uint8_t Q2CurveControl_PopCount(uint8_t value)
{
    uint8_t count = 0U;
...
static int16_t Q2CurveControl_Abs16(int16_t value)
{
    return (value < 0) ? (int16_t)(-value) : value;
}

static int16_t Q2CurveControl_Clamp(
    int16_t value,
    int16_t minimum,
    int16_t maximum)
{
...
static int16_t Q2CurveControl_Slew(
    int16_t current,
    int16_t target)
{
    if (target > current)
...
static uint8_t Q2CurveControl_IsRunState(CurveRaceState state)
{
...
static uint8_t Q2CurveControl_IsFinishCandidate(uint8_t filtered_mask)
{
...
static uint8_t Q2CurveControl_SelectApproachStage(uint32_t elapsed_ms)
{
...
static int16_t Q2CurveControl_SelectBasePwm(
    int16_t error_mm_q8,
    uint8_t approach_stage)
{
    int16_t magnitude = Q2CurveControl_Abs16(error_mm_q8);
...
static void Q2CurveControl_ResetDynamicControl(Q2CurveControl *control)
{
    Q2_PID_Reset(&control->line_pid);
...
void Q2CurveControl_Init(Q2CurveControl *control)
{
    ...
    Q2CurveControl_ResetDynamicControl(control);
}
...
void Q2CurveControl_PushSample1ms(
    Q2CurveControl *control,
    uint8_t raw_mask)
{
    uint8_t index;
    ...
    if (Q2CurveControl_IsRunState(control->output.state) != 0U)
    {
    ...
    {
        if (Q2CurveControl_IsFinishCandidate(raw_mask) != 0U)
        {
...
uint8_t Q2CurveControl_FilteredMask(const Q2CurveControl *control)
{
    ...
    for (index = 0U; index < CURVE_SENSOR_COUNT; index++)
    {
        if (Q2CurveControl_PopCount(control->history[index]) >= threshold)
        {
...
static void Q2CurveControl_RunLinePid(Q2CurveControl *control)
{
    int16_t base_pwm;
    int16_t turn_pwm;
    int16_t left_target;
    ...
    control->output.error_mm_q8 =
        Q2CurveControl_ErrorMmQ8(control->output.filtered_mask);
    control->output.approach_stage =
        Q2CurveControl_SelectApproachStage(
            control->output.elapsed_ms);
    base_pwm =
        Q2CurveControl_SelectBasePwm(
            control->output.error_mm_q8,
            control->output.approach_stage);
    ...
    Q2_PID_Update(&control->line_pid);
    turn_pwm = (int16_t)control->line_pid.Out;
    turn_pwm = Q2CurveControl_Clamp(
        turn_pwm,
    ...
    left_target =
        Q2CurveControl_Clamp(left_target, 0, CURVE_PWM_LIMIT);
    right_target =
        Q2CurveControl_Clamp(right_target, 0, CURVE_PWM_LIMIT);
    ...
    control->output.left_pwm =
        Q2CurveControl_Slew(control->output.left_pwm, left_target);
    control->output.right_pwm =
        Q2CurveControl_Slew(control->output.right_pwm, right_target);
}
...
Q2CurveControlOutput Q2CurveControl_Step5ms(Q2CurveControl *control)
{
    ...
    if (control == NULL)
    {
        Q2CurveControlOutput empty = {0};
        return empty;
    }

    control->output.filtered_mask =
        Q2CurveControl_FilteredMask(control);
    ...
    finish_candidate =
        Q2CurveControl_IsFinishCandidate(
            control->output.filtered_mask);
    ...
    Q2CurveControl_RunLinePid(control);
    return control->output;
}

Q2CurveControlOutput Q2CurveControl_GetOutput(
    const Q2CurveControl *control)
{
    Q2CurveControlOutput empty = {0};
